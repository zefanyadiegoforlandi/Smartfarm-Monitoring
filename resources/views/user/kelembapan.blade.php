...
<div class="min-w-full px-4 mb-5">
    ...
    <div class="items-center justify-between mt-5 flex">
        <div class="flex items-center justify-start">
            <p class="font-semibold text-3xl md:text-[40px] text-[#416D14]">Kelembapan</p>
        </div>
        ...
        <div class="bg-white border-transparent rounded-lg shadow-xl">
            ...
            <div class="p-5">
                ...
                <script>
                    ...
                    const suhu = tableData.map(entry => entry.kelembaban_udara);
    ...
    <table class="w-full  mt-5">
        <thead class="bg-[#ECF0E8]">
            <tr>
                ...
                <th class="hidden md:table-cell p-2 text-[#416D14] uppercase">Kelembapan</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($dataSensor as $k_udara)
            <tr class="text-center">
                <td class="p-2">{{ \Carbon\Carbon::parse($k_udara->waktu_perekaman)->format('H:i:s') }}</td>
                <td class="p-2">{{ \Carbon\Carbon::parse($k_udara->waktu_perekaman)->format('Y-m-d') }}</td>
                <td class="p-2">L007</td>
                <td class=" p-2">{{ $k_udara->kelembaban_udara }} %</td>
            </tr>
            @endforeach
